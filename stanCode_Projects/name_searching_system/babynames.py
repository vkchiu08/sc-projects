...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        year = ''                           # 年份要給他一個空字串裝第一個迴圈後的值，不然for loop會讓他在下個迴圈就不見了
        for line in f:
            word_lst = line.split(',')
            if len(line) == 5:              # 把txt的第一行抓出來當作年分，因為line後面多一個空白字元，所以不是4是5
                year = word_lst[0]
                year = year.strip()
            else:
                rank = word_lst[0]
...
def search_names(name_data, target):
    ...
    for key in name_data.keys():
        if target in key.upper():       # 為了case-insensitive
